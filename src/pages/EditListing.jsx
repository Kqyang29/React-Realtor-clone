...
function EditListing() {
	...
	const [lng, setLng] = useState(0);
		...
		useEffect(() => {
			setLoading(true);
			const listingRef = doc(db, "realtor_listing", param.id);
			const fetchList = async () => {
				const unsub = await onSnapshot(listingRef, (doc) => {
					if (doc.exists()) {
						...
						setFurnished(doc.data()?.furnished);
						setAddress(doc.data()?.address);
						setDescription(doc.data()?.description);
						setOffer(doc.data()?.offer);
						setRegularPrice(doc.data()?.regularPrice);
						setDiscountPrice(doc.data()?.discountPrice);
	...
	const handleSubmit = async (e) => {
		...
		if (geoLocationEnabled) {
		 ...
		 await updateDoc(doc(db, "realtor_listing",param.id), {
			name,
			offer,
			parking,
			geoLocation: {
		...
		selectedImg.map((image) => {
			const imageRef = ref(
				storage,
				`realtor_listing/${param.id}/${image.name}`
			);
		...
		navigate(`/category/${type}/${param.id}`);
	};
	...
	return (
		<div className="max-w-6xl mx-auto h-screen ">
			<div className="max-w-xl flex flex-col mx-auto p-5">
				<h1 className="text-3xl text-center font-semibold mb-8">Edit a List</h1>
